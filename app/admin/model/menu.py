...
class Menu(Base):
    ...
    component: Mapped[str | None] = mapped_column(String(255), default=None, comment='menu component')
    perms: Mapped[str | None] = mapped_column(String(100), default=None, comment='menu permissions')
    status: Mapped[int] = mapped_column(default=1, comment='Menu status (0 disabled 1 normal)')
    display: Mapped[int] = mapped_column(default=1, comment='Menu display (0 hidden 1 shown)')
    cache: Mapped[int] = mapped_column(default=1, comment='Cache menu (0 not cached 1 cached)')
