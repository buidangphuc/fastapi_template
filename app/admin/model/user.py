...
class User(Base):
    ...
    salt: Mapped[bytes | None] = mapped_column(
        VARBINARY(255).with_variant(BYTEA(255), "postgresql"), comment="password salt"
    )
    email: Mapped[str] = mapped_column(
        String(50), unique=True, index=True, comment="user email"
    )
    is_superuser: Mapped[bool] = mapped_column(
        Boolean().with_variant(INTEGER, "postgresql"),
        default=False,
        comment="Super user (0: no, 1: yes)",
    )
    is_staff: Mapped[bool] = mapped_column(
        Boolean().with_variant(INTEGER, "postgresql"),
        default=False,
        comment="Staff (0: no, 1: yes)",
    )
    status: Mapped[int] = mapped_column(
        default=1, index=True, comment="User status (0: disabled, 1: normal)"
    )
    is_multi_login: Mapped[bool] = mapped_column(
        Boolean().with_variant(INTEGER, "postgresql"),
        default=False,
        comment="Allow multiple logins (0: no, 1: yes)",
    )
    avatar: Mapped[str | None] = mapped_column(
        String(255), default=None, comment="user avatar"
    )
    phone: Mapped[str | None] = mapped_column(
        String(11), default=None, comment="user phone"
    )
    join_time: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        init=False,
        default_factory=timezone.now,
        comment="user join time",
    )
