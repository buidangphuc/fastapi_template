...
class OperaLog(DataClassBase):
    ...
    status: Mapped[int] = mapped_column(comment='operation status (0: success, 1: failure)')
    code: Mapped[str] = mapped_column(String(20), insert_default='200', comment='response code')
    msg: Mapped[str | None] = mapped_column(LONGTEXT().with_variant(TEXT, 'postgresql'), comment='message')
    cost_time: Mapped[float] = mapped_column(insert_default=0.0, comment='cost time (ms)')
