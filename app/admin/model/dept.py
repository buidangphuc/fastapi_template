...
class Dept(Base):
    ...
    leader: Mapped[str | None] = mapped_column(
        String(20), default=None, comment="leader name"
    )
    phone: Mapped[str | None] = mapped_column(
        String(11), default=None, comment="leader phone"
    )
    email: Mapped[str | None] = mapped_column(
        String(50), default=None, comment="leader email"
    )
    status: Mapped[int] = mapped_column(
        default=1, comment="Department status (0 disabled 1 normal)"
    )
    del_flag: Mapped[bool] = mapped_column(
        Boolean().with_variant(INTEGER, "postgresql"),
        default=False,
        comment="Delete flag (0 to delete, 1 to exist)",
    )
    ...
    parent: Mapped[Optional["Dept"]] = relationship(
        init=False, back_populates="children", remote_side=[id]
    )
    children: Mapped[Optional[list["Dept"]]] = relationship(
