...
async def add_user(
    request: Request, obj: AddUserParam
...
@router.put(
    "/{username}/avatar", summary="Update profile avatar", dependencies=[DependsJwtAuth]
)
...
async def super_set(
    request: Request, pk: Annotated[int, Path(description="user ID")]
) -> ResponseModel:
...
@router.put(
    "/{pk}/staff",
    summary="Modify user backend login permissions",
...
@router.put(
    "/{pk}/multi",
    summary="Modify the user's multi-terminal login status",
    dependencies=[DependsRBAC],
)
async def multi_set(
    request: Request, pk: Annotated[int, Path(description="user ID")]
