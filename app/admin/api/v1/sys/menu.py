...
@router.get(
    "/sidebar",
    summary="Get the user menu sidebar",
    description="Adapt vben5",
    dependencies=[DependsJwtAuth],
)
async def get_user_sidebar(
    request: Request,
...
async def get_menu(
    pk: Annotated[int, Path(description="menu ID")]
...
async def update_menu(
    pk: Annotated[int, Path(description="menu ID")], obj: UpdateMenuParam
) -> ResponseModel:
...
async def delete_menu(
    pk: Annotated[int, Path(description="menu ID list")]
) -> ResponseModel:
