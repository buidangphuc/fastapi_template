...
@router.get(
    "/models", summary="Get data rule available models", dependencies=[DependsJwtAuth]
)
...
@router.get(
    "/model/{model}/columns",
    summary="Get the available model columns for data rules",
...
async def delete_data_rule(
    pk: Annotated[list[int], Query(description="Data rule ID list")]
) -> ResponseModel:
