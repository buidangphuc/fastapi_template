...
async def get_current_user(db: AsyncSession, pk: int) -> User:
    ...
    if not user:
        raise TokenError(msg='Invalid token')
    if not user.status:
        raise AuthorizationError(msg='User is locked, please contact system administrator')
    if user.dept_id:
        if not user.dept.status:
            raise AuthorizationError(msg='User\'s department is locked, please contact system administrator')
        if user.dept.del_flag:
            raise AuthorizationError(msg='User\'s department has been deleted, please contact system administrator')
    if user.roles:
        role_status = [role.status for role in user.roles]
        if all(status == 0 for status in role_status):
            raise AuthorizationError(msg='User\'s roles are locked, please contact system administrator')
    return user
