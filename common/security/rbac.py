...
async def rbac_verify(request: Request, _token: str = DependsJwtAuth) -> None:
    ...
    if not user_roles or all(status == 0 for status in user_roles):
        raise AuthorizationError(msg='User has no assigned roles, please contact system administrator')

    # Check user role menus
    if not any(len(role.menus) > 0 for role in user_roles):
        raise AuthorizationError(msg='User has no assigned menus, please contact system administrator')

    # Check backend management operation permissions
    method = request.method
    if method != MethodType.GET or method != MethodType.OPTIONS:
        if not request.user.is_staff:
            raise AuthorizationError(msg='User is forbidden from backend operations, please contact system administrator')
    ...
    if settings.RBAC_ROLE_MENU_MODE:
        ...
        try:
            ...
            log.error(f'Attempting to perform RBAC permission verification through casbin, but this plugin does not exist: {e}')
